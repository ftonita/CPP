...
void Scalar::parse()
{
	...
	if (_str.length() > 1) {
		...
		if (static_cast<unsigned long>(i) == _str.length())
			throw std::string("EMPTY ARGUMENT");
		_str = _str.substr(i);
		...
		if (_str.length() > 1) {
			for (i = 0; static_cast<unsigned long>(i) < _str.length(); ++i) {
				if (_str[i] == '.') {
					if (_isPoint == true || static_cast<unsigned long>(i) == _str.length() - 1)
						throw std::string("IS NOT DIGIT");
					_isPoint = true;
				}
			...
			if (static_cast<unsigned long>(i) != _str.length()) {
				if (!(static_cast<unsigned long>(i + 1) == _str.length() && _str[i] == 'f' && _isPoint == true))
					throw std::string("IS NOT DIGIT");
			}
